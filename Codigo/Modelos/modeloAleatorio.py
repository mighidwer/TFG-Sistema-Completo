...
----------------------------------------------------------------------------------------------------
 ...
 · modeloAleatorio.py
    ...
    Este fichero implementa un modelo alternativo basado en la asignación
    aleatoria de visitas a las tiendas en cada jornada.
...
'''
    Construye el modelo aleatorio, basado en una estrategia de visitas en la que las tiendas
    se recorren siguiendo un orden fijo prefijado (generado aleatoriamente para cada jornada).
...
def ejecutar(datos, numComerciantesFijo=None, diaIteracionMPC=0):
    ...
    return modeloBase.ejecutar(datos, generarVisitasRutasAleatorio, numComerciantesFijo, diaIteracionMPC)
...
'''
    ...
    a partir de una asignación aleatoria de tiendas a los comerciantes.
...
'''
def generarVisitasRutasAleatorio(numComerciantes, numDias, numTiendas, distanciaRutas, tiempoRutas, distanciaMax, tiempoMax, tiempoVisita, frecuenciaVisita, diaIteracionMPC=0):
    
    random.seed(27)  # Para asignar las mismas rutas en todas las simulaciones
    secuencias = {}
    ...
    for d in range(numDias):
        ...
        # Creamos el orden de visitas para cada día
