...
----------------------------------------------------------------------------------------------------
 ...
 · modeloGreedy.py
    ...
    Este fichero implementa un modelo heurístico basado en un criterio greedy,
    en el que se prioriza la visita a las tiendas más cercanas en cada etapa.
...
'''
    Construye el modelo greedy, basado en una estrategia de visitas en la que las tiendas
    se recorren siguiendo un orden fijo prefijado (según su cercanía a la última tienda).
...
def ejecutar(datos, numComerciantesFijo=None, diaIteracionMPC=0):
    ...
    return modeloBase.ejecutar(datos, generarVisitasRutasGreedy, numComerciantesFijo, diaIteracionMPC)
...
'''
    ...
    utilizando una heurística greedy basada en la mínima distancia a la siguiente tienda.
...
'''
def generarVisitasRutasGreedy(numComerciantes, numDias, numTiendas, distanciaRutas, tiempoRutas, distanciaMax, tiempoMax, tiempoVisita, frecuenciaVisita, diaIteracionMPC=0):
    ...
    for d in range(numDias):
        ...
        pendientes = set(range(1, numTiendas))
        orden = []
        
        while pendientes:
            
            # Cuando se empieza con la ruta de un nuevo comerciante, partimos desde la oficina
            distanciaAcum = 0
            tiempoAcum = 0
            actual = 0
            ...
            while pendientes:
                ...
                siguiente = min(pendientes, key=lambda i: distanciaRutas[actual][i])
                
                # Comprobamos que la ruta sigue siendo válida, contando con la vuelta a la oficina
                extraDist = distanciaRutas[actual][siguiente] + distanciaRutas[siguiente][0]
                extraTime = tiempoRutas[actual][siguiente] + tiempoRutas[siguiente][0] + tiempoVisita
                
                # Si no cabe una tienda más, terminamos la ruta
                if distanciaAcum + extraDist > distanciaMax or tiempoAcum + extraTime > tiempoMax:
                    break

                orden.append(siguiente)
                pendientes.remove(siguiente)

                distanciaAcum += distanciaRutas[actual][siguiente]
                tiempoAcum += tiempoRutas[actual][siguiente] + tiempoVisita

                actual = siguiente
                
        # Asignamos las tiendas a visitar para los días correspondientes
